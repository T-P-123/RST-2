...
| Délka úseku 2 | $L_2$ | $80\ \mathrm{mm}$ |
| Délka úseku 3 | $L_3$ | $60\ \mathrm{mm}$ |
| Celková délka | $L$ | $L_1+L_2+L_3 = 200\ \mathrm{mm}$ |
...
$$
\sigma_{\max,1} = K_{t1}\cdot\sigma_{\mathrm{nom},2} = 1{,}68 \cdot 44{,}44 = 74{,}7\ \mathrm{MPa}
$$

$$
\sigma_{\max,2} = K_{t2}\cdot\sigma_{\mathrm{nom},3} = 1{,}15 \cdot 400 = \boxed{460\ \mathrm{MPa}}
$$

**Rozhoduje vrub $r_2$.** Přestože má nižší $K_t$, vyšší nominální napětí v úseku 3 dominuje.

### 3.5 Součinitel bezpečnosti k MS pružnosti

$$
k_{\mathrm{yield}} = \frac{R_e}{\sigma_{\max,2}} = \frac{500}{460} = \boxed{1{,}087}
$$

**Velmi malá rezerva** — pouhých 8,7 % do meze kluzu. Při i malé výrobní odchylce součást poteče.
...
S $L = 200\ \mathrm{mm}$, hranice úseků $[0; 60]$, $[60; 140]$, $[140; 200]$:

$$
J_1 = \frac{(200-0)^2 - (200-60)^2}{2} = \frac{40\,000 - 19\,600}{2} = 10\,200
$$

$$
J_2 = \frac{(200-60)^2 - (200-140)^2}{2} = \frac{19\,600 - 3\,600}{2} = 8\,000
$$

$$
J_3 = \frac{(200-140)^2 - (200-200)^2}{2} = \frac{3\,600 - 0}{2} = 1\,800
$$
...
|------|-----------|-------------------------------------------|-------|
| 1 | $10\,200/113\,906 = 0{,}0896$ | $0{,}043$ | 5,1 % |
| 2 | $8\,000/33\,750 = 0{,}2370$ | $0{,}113$ | 13,4 % |
| 3 | $1\,800/1\,250 = 1{,}4400$ | $\mathbf{0{,}686}$ | **81,5 %** |

$$
\boxed{w = 0{,}043 + 0{,}113 + 0{,}686 = 0{,}842\ \mathrm{mm}}
$$

> **Důležité pozorování:** **Úsek 3 přispívá 81 %** k celkovému průhybu, přestože je nejkratší. Důvod: závislost na $1/I \propto 1/h^3$ — třetí mocnina znamená, že tenký úsek je ohybově extrémně poddajný.

#### Součinitel bezpečnosti k MS deformace

$$
k_{\mathrm{deform}} = \frac{w_{\max}}{w} = \frac{2}{0{,}842} = 2{,}38
$$

Více než 2× rezerva — deformace deterministicky vyhovuje s velkým komfortem.

### 3.7 Shrnutí deterministických výsledků

| Veličina | Hodnota | Vyhodnocení |
|----------|---------|-------------|
| $\sigma_{\max}$ | $460\ \mathrm{MPa}$ | < $R_e = 500\ \mathrm{MPa}$ ✓ ($k=1{,}087$) |
| $w$ | $0{,}842\ \mathrm{mm}$ | < $w_{\max} = 2\ \mathrm{mm}$ ✓ ($k=2{,}38$) |

Deterministicky **oba mezní stavy vyhovují**, ale bezpečnost u MS pružnosti je velmi malá (8,7 %).
...
| $K_{t2}$ | $1{,}15$ | LN, CoV=0,1 | Vrubový součinitel |
| $L$ | $200\ \mathrm{mm}$ | LN, CoV=0,1 | Celková délka |
...
|------------|-------|---------|-------------------------|-------------|
| **MS pružnosti** | $\boxed{39{,}6\ \%}$ | $0{,}26$ | $\mu(\sigma_{\max}) \approx 470\ \mathrm{MPa}$, $\sigma \approx 128\ \mathrm{MPa}$ | **Nevyhovuje** |
| **MS deformace** | $\boxed{0{,}47\ \%}$ | $2{,}60$ | $\mu(w) \approx 0{,}87\ \mathrm{mm}$ | Vyhovuje s rezervou |

### 4.8 Klíčový závěr — proč je $P_f$ pro pružnost tak velké?

**Deterministicky** $\sigma_{\max} = 460\ \mathrm{MPa}$ leží **těsně pod** $R_e = 500\ \mathrm{MPa}$ — mezera pouhých 40 MPa.

**Stochasticky** je směrodatná odchylka výstupu $\sigma_{\sigma_{\max}} \approx 128\ \mathrm{MPa}$, tedy **více než třikrát** větší než tato bezpečnostní rezerva.
...
**Závěr:** Součást **deterministicky projde**, ale stochasticky má **40% pravděpodobnost překročení meze kluzu** — z hlediska pravděpodobnostního inženýrství **nevyhovuje**.
...
|----------|------------------|--------------|--------------|
| $h_3$ | $+0{,}683$ | $\mathbf{48{,}8\ \%}$ | **Dominantní** — výška kritického průřezu |
| $R_e$ | $+0{,}365$ | $13{,}9\ \%$ | Mez kluzu |
| $M$ | $-0{,}345$ | $12{,}5\ \%$ | Zatížení |
| $K_{t2}$ | $-0{,}345$ | $12{,}4\ \%$ | Vrubový součinitel |
| $t$ | $+0{,}343$ | $12{,}3\ \%$ | Tloušťka |
...
| **$\sigma_{\max}$ deterministicky** | $460\ \mathrm{MPa}$ (mezera $40\ \mathrm{MPa}$ do $R_e$) |
| **$w$ deterministicky** | $0{,}842\ \mathrm{mm}$ (úsek 3 dává 81 % příspěvku) |
| **MS pružnosti — $P_f$** | $\mathbf{39{,}6\ \%}$ → součást **NEVYHOVUJE** |
| **MS deformace — $P_f$** | $0{,}47\ \%$ → vyhovuje ($\beta = 2{,}6$) |
| **Dominantní citlivost** | $h_3$ — 49 % rozptylu (kvadratická závislost) |
| **Doporučení** | Zvětšit $h_3$ nebo zpřesnit jeho toleranci |

**Hlavní poučení:** Deterministická bezpečnost $k = 1{,}087$ vypadá uspokojivě, ale **při běžných výrobních tolerancích ($\mathrm{CoV} = 10\,\%$) je 40 % pravděpodobnost plastické deformace**. Pravděpodobnostní přístup tedy odhaluje slabé místo, které deterministický výpočet skrývá.
