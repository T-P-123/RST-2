...
\begin{itemize}[nosep]
    \item \textbf{Vlevo:} pevný svar k~základnímu rámu (přenese tah i tlak).
    \item \textbf{Vpravo:} kontakt s~rámem ($\delta = 0$) — přenese pouze tlak,
          při tahu se rozevře.
\end{itemize}

Vnější souměrně střídavá síla $F = \pm 1000\unit{N}$. Stanovte časový průběh napětí ve vrubech.
...
\begin{itemize}[nosep]
    ...
    \item Údaj $1{,}4 \times 10^5\unit{N}$ z výkresu — význam neověřen
          (pravděpodobně tuhost kontaktu nebo značení svaru, ne axiální předpětí).
\end{itemize}

\subsection{Deterministické řešení}

\subsubsection{Vliv jednostranného kontaktu — degenerace cyklu}

Asymetrická vazba (svar vs.~kontakt) rozštěpí cyklus síly $F$ na dvě poloviny:

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Polosmysl $F$} & \textbf{Pravý kontakt} & \textbf{Vnitřní síla v~prutu} \\
\midrule
$F$ táhne (od rámu) & rozevřen        & $N = F = 1000\unit{N}$ (tah) \\
$F$ tlačí (do rámu) & sevřen, přenese & $N = 0$ \\
\bottomrule
\end{tabular}
\end{center}

Souměrně střídavé vnější zatížení tedy degeneruje na \textbf{pulzující (míjivý) cyklus} ve vrubu:
$$\sigma_{\min} = 0, \quad R = \sigma_{\min}/\sigma_{\max} = 0.$$
...
\begin{align}
    \frac{D}{d} &= \frac{50}{35} = 1{,}429, \qquad \frac{r}{d} = \frac{5}{35} = 0{,}143
\end{align}

Z~Petersonova diagramu:
\begin{equation}
    \boxed{K_t \approx 2{,}10} \quad (\text{rozsah } 2{,}0\text{--}2{,}2)
\end{equation}

\textbf{Pozn.:} Neuberova aproximace $K_t = 1 + 2\sqrt{t/r} = 3{,}449$ je pro tuto geometrii konzervativní horní mez. Petersonův graf vychází z~přesnějšího řešení teorie pružnosti a~je standardně používán.

\subsubsection{Napětí v~kořeni vrubu}

\begin{align}
    \sigma_{\mathrm{nom,max}} &= \frac{F_{\max}}{A_d} = \frac{1000}{962{,}1} = 1{,}039\unit{MPa} \\
    \sigma_{\max} &= K_t \cdot \sigma_{\mathrm{nom,max}} = 2{,}10 \cdot 1{,}039 = \boxed{2{,}18\unit{MPa}} \\
    \sigma_{\min} &= 0\unit{MPa} \quad (R = 0)
\end{align}

Střední napětí a~amplituda:
\begin{align}
    \sigma_m &= \frac{\sigma_{\max} + \sigma_{\min}}{2} = 1{,}09\unit{MPa} \\
    \sigma_a &= \frac{\sigma_{\max} - \sigma_{\min}}{2} = 1{,}09\unit{MPa}
\end{align}
...
Napětí harmonicky kmitá mezi $0$ a~$\sigma_{\max} \approx 2{,}18\unit{MPa}$ s~poměrem asymetrie $R = 0$, perioda shodná s~periodou síly $F$.
...
\midrule
$\sigma_{\max}$ [MPa] & 2{,}183 & 0{,}034 & 0{,}015 & $[2{,}12;\; 2{,}25]$ \\
$\sigma_a$ [MPa] & 1{,}092 & 0{,}017 & 0{,}015 & --- \\
$K_t$ [-] & 2{,}100 & 0{,}020 & 0{,}010 & --- \\
\bottomrule
\end{tabular}
\end{table}

Variabilita napětí je velmi nízká ($\mathrm{CoV} \approx 1{,}5\%$) díky malým tolerancím rozměrů a~slabé citlivosti $K_t$ na geometrii v~okolí pracovního bodu.
...
\midrule
\textbf{5E} & $\sigma_{\max} = 2{,}18\unit{MPa}$, $K_t = 2{,}10$ (Peterson), $R = 0$ & CoV napětí: $0{,}015$ & $\pm 0{,}5\unit{mm}$ \\
\bottomrule
\end{tabular}
\end{table}
